...
def get_random_avatar():
	avatar_list = [
         '902000013', '902049020', '902049019', '902049018', '902049017', '902049016', '902047016', '902027027', '902027022','902000207', '902000065', '902048020', '902043017', '902043018', '902043019', '902043020', '902043021', '902043022', '902043023', '902043024','902043025', '902045016', '902045017', '902045018', '902045019', '902045020', '902045021', '902045022', '902045023', '902045024', '902045025', '902037031'
    ]
                ...
                if "1200" in data.hex()[0:4] and b"/sm" in data:
                    try:
                        ...
                        if match:
                            ...
                            spam_count = 100  # You can adjust this value
...
#-------------------------------------------------------------#          
                if "1200" in data.hex()[0:4] and b"[FFFFF00]TOM  [ffffff]XR" in data:
                    pass
                else:
...
[FFFFFF]⚡lag Any Team
[00FFFF]⚡/🙃lag [teamcode]
[FFFFFF]⚡lag 2 Any Team
[00FFFF]⚡/🙃lag [teamcode] 2
[FFFFFF]⚡Force Start a Team 
[00FFFF]⚡/🙃start [teamcode]""", uid
                                    )
                                )
                            time.sleep(0.5)
                            clients.send(
...
#-------------------------------------------------------------#
                if '1200' in data.hex()[0:4] and b'/join' in data:
                    try:
                        ...
                        clients.send(
                            self.GenResponsMsg(f"[C][B][32CD32]Attempting to join team: {room_id}", uid)
                        )
                        ...
                        clients.send(
                            self.GenResponsMsg(f"[C][B][00FF00]Successfully joined the team.", uid)
                        )
...
#-------------------------------------------------------------#                                
                if "1200" in data.hex()[0:4] and b"/ev" in data:
                    try:
                        ...
                        restart_program()
#-------------------------------------------------------------#                                 
                if "1200" in data.hex()[0:4] and b'/em' in data:
                    try:
                        json_result = get_available_room(data.hex()[10:])
                        parsed_data = json.loads(json_result)
                        uid_sender = parsed_data["5"]["data"]["1"]["data"]
                        ...
                        command_parts = data.split(b'/em')[1].split(b'(')[0].decode().strip().split()
                        if len(command_parts) < 2:
                            clients.send(self.GenResponsMsg("[C][B][FF0000]Usage: /🙃em <target_id> <emote_id>", uid_sender))
                            continue
                        ...
                        logging.error(f"Error processing /🙂em command: {e}")
                        try:
